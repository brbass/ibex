...
Strong_Basis_Scattering::
Strong_Basis_Scattering(shared_ptr<Weak_Spatial_Discretization> spatial_discretization,
                ...
                for (int n = 0; n < number_of_nodes; ++n)
                {
                    ...
                    for (int j = 0; j < number_of_basis_functions; ++j)
                    {
                        ...
                        vector<double> const sigma_s
                            = spatial_discretization_->weight(b)->material()->sigma_s()->data();
                        ...
                        for (int gf = 0; gf < number_of_groups; ++gf)
                        {
                            ...
                            sum += sigma_s[k_sigma] * mult * y[k_phi_from];
                        } // from group
                    } // basis functions
...
{
    AssertMsg(false, "coherent scattering not yet implemented in Strong_Basis_Scattering");
}
