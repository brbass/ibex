...
void output_error(XML_Node output_node,
                  ...
                  shared_ptr<Manufactured_Heat_Data> data,
                  shared_ptr<Heat_Transfer_Solution> solution)
{
    ...
    for (int i = 0; i < number_of_ordinates; ++i)
    {
        ...
        double analytic = data->solution(position);
    ...
    for (int i = 0; i < number_of_points; ++i)
    {
        ...
        benchmark[i] = data->solution(position);
    }
              ...
              XML_Node output_node)
{
    ...
    string manufactured_type
        = heat_node.get_attribute<string>("type");
    shared_ptr<Manufactured_Heat_Data> data;

    if (manufactured_type == "sin")
    {
        vector<double> sol_coeff
            = heat_node.get_child_vector<double>("solution",
                                                 2);
        ...
        vector<double> conv_coeff
            = heat_node.get_child_vector<double>("convection",
                                                 3);
        data
            = make_shared<Manufactured_Sin_Data>(dimension,
                                                 limits,
                                                 sol_coeff,
                                                 cond_coeff,
                                                 conv_coeff);
    ...
    {
        AssertMsg(false, "manufactured type (" + manufactured_type + ") not found");
    }
    // Perform integration
